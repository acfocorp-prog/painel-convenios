...
export function MandatoDetailPage() {
  ...
  const { id } = useParams();
  const { data: m, isLoading } = useMandatoById(id);
  const { data: statusCatalog } = useStatusCatalog();
  const update = useUpdateMandato(id ?? '');
  const del = useDeleteMandato();
  const { data: history } = useStatusHistory('MANDATO', id);
  ...
  async function togglePriority() {
    try {
      await update.mutateAsync({ priority: !m!.priority });
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Erro');
    }
  }
  ...
  async function onDelete() {
    if (!confirm('Excluir este mandato? (pode ser restaurado depois)')) return;
    try {
  ...
  return (
    <div className="space-y-4 pb-6">
      <div className="-mx-4">
        <PageHeader
          title={isSecretaria ? 'Mandato tampão — Secretaria' : (m.escolas?.name ?? 'Mandato tampão')}
          description="Mandato interino"
          action={
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                size="icon"
                aria-label="Editar"
                onClick={() => navigate(`/mandatos/${id}/editar`)}
              >
      ...
      <div className="space-y-3 px-4">
        <Card>
          <CardContent className="space-y-3 pt-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <Info icon={Tag} label="Início">
                {formatDate(m.start_date)}
              </Info>
              <Info icon={Tag} label="Fim">
              ...
              <Info icon={isSecretaria ? Building2 : School} label="Vínculo">
                {isSecretaria
                  ? 'Mandato da secretaria'
                  : `${m.escolas?.name ?? 'Escola removida'} · INEP ${m.escolas?.inep ?? '—'}`}
              </Info>
            </div>
            ...
            {m.escolas && (
              <button
                type="button"
                onClick={() => navigate(`/escolas/${m.escolas!.id}`)}
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            variant={m.priority ? 'success' : 'outline'}
            size="sm"
            onClick={togglePriority}
            disabled={update.isPending}
          >
            {m.priority ? '✓ É prioridade' : 'Marcar como prioritário'}
          </Button>
